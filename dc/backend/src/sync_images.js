...
async function main() {
  ...
  try {
    ...
    if (downloadCount > 0) {
      console.log(`[AutoSync] ดาวน์โหลดรูปล่าสุดเสร็จสิ้น: ${downloadCount} รูป ลงใน D:\\รูป`);
    }
...
function startAutoSync() {
  console.log("[AutoSync] ระบบดึงรูปอัตโนมัติแบบเบื้องหลังเริ่มทำงานแล้ว (ดึงลง D:\\รูป)...");
  setInterval(async () => {
    try {
      await main();
    } catch (e) {}
