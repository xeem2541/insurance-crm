...
router.post('/extract', authenticateToken, upload.array('images', 10), async (req, res) => {
  try {
    ...
    5. ข้อมูลรถยนต์ (Vehicle Info):
       ...
       - sum_insured: ทุนประกันภัยของตัวรถยนต์ (ให้ตรวจดูจากจำนวนเงินความคุ้มครองในหมวด "ความเสียหายต่อรถยนต์" หรือ "รถยนต์สูญหาย/ไฟไหม้" เป็นหลัก) ดึงเฉพาะตัวเลขเพียวๆ ไม่มีลูกน้ำ
    ...
    โครงสร้าง JSON ที่ต้องการส่งกลับ (ห้ามเปลี่ยนชื่อ Key เด็ดขาด):
    {
      ...
      "vehicle": {
        ...
        "sum_insured": "ทุนประกันภัยของรถ (ตัวเลขเพียวๆ)"
      },
      "policy": {
    ...
    "sum_insured": "ทุนประกันภัย (ให้ดูจากความคุ้มครองความเสียหายต่อรถยนต์ หรือรถยนต์สูญหาย/ไฟไหม้ เป็นหลัก และดึงเฉพาะตัวเลขเพียวๆ)",
