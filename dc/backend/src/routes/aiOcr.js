...
router.post('/extract', authenticateToken, upload.array('images', 10), async (req, res) => {
  try {
    ...
    2. การแยกที่อยู่อย่างชาญฉลาด (Thai Address Parser):
       ...
       - address: บ้านเลขที่ เท่านั้น (เฉพาะบ้านเลขที่ เช่น "12/34" หรือ "99/1" ห้ามใส่ หมู่, ซอย, ถนน หรืออาคาร เข้ามารวมในฟิลด์นี้เด็ดขาด)
       - moo: หมู่ หรือ หมู่ที่ (เช่น "5" หรือ "หมู่ที่ 5" หากไม่มีให้ใส่ "")
       - soi: ซอย (เช่น "สุขุมวิท 3" หรือ "ซอย 3" หากไม่มีให้ใส่ "")
       - road: ถนน (เช่น "สุขุมวิท" หรือ "ถนนสุขุมวิท" หากไม่มีให้ใส่ "")
    ...
    5. ข้อมูลรถยนต์ (Vehicle Info):
       ...
       - sum_insured: ทุนประกันภัยของตัวรถยนต์ (ให้ตรวจดูจากจำนวนเงินความคุ้มครองในหมวด "ความเสียหายต่อรถยนต์" หรือ "รถยนต์สูญหาย/ไฟไหม้" เท่านั้น ห้ามสับสนกับความรับผิดต่อบุคคลภายนอกที่มีมูลค่า 500,000 หรือ 1,000,000 บาท) ดึงเฉพาะตัวเลขเพียวๆ ไม่มีลูกน้ำ
       - color: สีรถยนต์ แปลงเป็นภาษาไทยให้ตรงกับตัวเลือก เช่น ขาว, ดำ, เทา, บรอนซ์เงิน, บรอนซ์ทอง, แดง, น้ำเงิน, ฟ้า, น้ำตาล, เขียว, เหลือง, ส้ม, ชมพู (หากไม่มีให้ใส่ "")
    ...
    โครงสร้าง JSON ที่ต้องการส่งกลับ (ห้ามเปลี่ยนชื่อ Key เด็ดขาด):
    {
      "customer": {
        ...
        "phone": "เบอร์โทรศัพท์ของผู้เอาประกันภัย/ลูกค้า (ห้ามดึงเบอร์ของบริษัทประกัน หรือเบอร์ของตัวแทน/นายหน้า ให้ตรวจหาเบอร์ของตัวลูกค้าเอง เช่น 08x-xxxxxxx)",
        "dob": "วันเดือนปีเกิด (YYYY-MM-DD)",
        "address": "บ้านเลขที่ (เฉพาะบ้านเลขที่เท่านั้น ห้ามมีคำว่า หมู่ หรือซอย ถนน)",
      ...
      "vehicle": {
        ...
        "sum_insured": "ทุนประกันภัยของรถ (ให้ตรวจดูจากความคุ้มครองความเสียหายต่อรถยนต์ หรือรถยนต์สูญหาย/ไฟไหม้ เท่านั้น ห้ามดึงค่าความรับผิดต่อบุคคลภายนอก 500,000 หรือ 1,000,000 เด็ดขาด)"
      },
      "policy": {
        ...
        "sum_insured": "ทุนประกันภัย (ให้ดูจากความคุ้มครองความเสียหายต่อรถยนต์ หรือรถยนต์สูญหาย/ไฟไหม้ เท่านั้น ห้ามดึงค่าความรับผิดต่อบุคคลภายนอก 500,000 หรือ 1,000,000 เด็ดขาด)",
        ...
        "start_date": "วันเริ่มต้นคุ้มครอง (YYYY-MM-DD) (ให้ตรวจหาจากหมวด 'ระยะเวลาประกันภัย' หรือ 'ระยะเวลาเอาประกันภัย' เป็นหลัก ห้ามดึงข้อมูลวัน พ.ร.บ. เว้นแต่เป็นกรมธรรม์ พ.ร.บ.)",
        "expiry_date": "วันสิ้นสุดคุ้มครอง (YYYY-MM-DD) (ให้ตรวจหาจากหมวด 'ระยะเวลาประกันภัย' หรือ 'ระยะเวลาเอาประกันภัย' เป็นหลัก ห้ามดึงข้อมูลวัน พ.ร.บ. เว้นแต่เป็นกรมธรรม์ พ.ร.บ.)"
      }
    }`;
