...
router.post('/extract', authenticateToken, upload.array('images', 10), async (req, res) => {
  ...
  try {
    ...
    3. การกรองและจัดรูปแบบชื่อและคำนำหน้าลูกค้า (Customer Name & Info):
       ...
       - **เบอร์โทรศัพท์ (phone):** ดึงเฉพาะเบอร์โทรศัพท์ของลูกค้าผู้เอาประกันภัยเท่านั้น ส่งเฉพาะตัวเลขล้วน (ความยาว 9-10 หลัก ไม่มีขีดและช่องว่าง) *ห้ามดึงเบอร์โทรศัพท์ของบริษัทประกันภัย (เช่น 1557, 1484, 02-xxx-xxxx) หรือเบอร์ตัวแทน/นายหน้ามาใส่เป็นเบอร์โทรลูกค้าอย่างเด็ดขาด*
    ...
    const modelsToTry = [
      { name: 'gemini-3.1-flash-lite', timeout: 12000 },  // Primary: High quota (500 RPD)
      { name: 'gemini-2.5-flash', timeout: 10000 },       // Fallback: 20 RPD
      { name: 'gemini-2.5-flash-lite', timeout: 8000 },   // Fallback: 20 RPD
      { name: 'gemini-3.5-flash', timeout: 12000 }        // Fallback: 20 RPD (Current limit exceeded, keep last)
    ];
