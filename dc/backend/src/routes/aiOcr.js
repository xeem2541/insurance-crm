...
router.post('/extract', authenticateToken, upload.array('images', 10), async (req, res) => {
  ...
  try {
    ...
    5. วันที่และวันคุ้มครอง (Date Handling):
       ...
       - start_date (วันเริ่มต้นคุ้มครอง): ให้ดึงจากหัวข้อ 'ระยะเวลาเอาประกันภัย' (Period of Insurance) หรือ 'ระยะเวลาประกันภัย' -> 'เริ่มต้นวันที่' หรือ 'จากวันที่' (ปกติระบุเวลาเริ่มต้น 16.30 น. หรือ 00.01 น.) ของตัวตารางหลักประกันภัยรถยนต์สมัครใจ ห้ามดึงวันจดทะเบียนรถ หรือวันออกเอกสาร หรือวันออกกรมธรรม์ หรือวันคุ้มครองของ พ.ร.บ. มาใส่ปนเด็ดขาด
       - expiry_date (วันสิ้นสุดคุ้มครอง): ให้ดึงจากหัวข้อ 'ระยะเวลาเอาประกันภัย' หรือ 'ระยะเวลาประกันภัย' -> 'สิ้นสุดวันที่' หรือ 'ถึงวันที่' (ปกติระบุเวลาสิ้นสุด 16.30 น.) ของตัวตารางหลักประกันภัยรถยนต์สมัครใจ ห้ามดึงวันภาษีสิ้นสุด หรือวันจดทะเบียนรถ หรือวันคุ้มครองของ พ.ร.บ. มาใส่ปนเด็ดขาด
    ...
    7. ข้อมูลรถยนต์ (Vehicle Info):
       ...
       - sum_insured: **ทุนประกันภัยของตัวรถยนต์** (ค้นหาตัวเลขจำนวนเงินเอาประกันภัยสำหรับตัวรถยนต์เท่านั้น โดยปกติจะระบุอยู่ในตารางความรับผิดต่อตัวรถ เช่น 'ความเสียหายต่อรถยนต์' หรือ 'รถยนต์สูญหาย/ไฟไหม้' เท่านั้น มูลค่ามักอยู่ระหว่าง 100,000 ถึง 2,000,000 บาท ห้ามดึงค่าความรับผิดต่อบุคคลภายนอก เช่น 500,000 หรือ 10,000,000 เด็ดขาด และหากเป็นประกันชั้น 3 หรือเอกสาร พ.ร.บ. ให้ระบุเป็น 0)
       ...
       - vin (เลขตัวถัง) และ engine_no (เลขเครื่องยนต์): ดึงมาอย่างละเอียดและแม่นยำ 100% ห้ามมีเครื่องหมายขีด (-) หรือเว้นวรรคปะปนเด็ดขาด ส่งเฉพาะรหัสตัวเลขผสมภาษาอังกฤษเท่านั้น (เลขตัวถังปกติมี 17 หลัก)
    ...
    9. กฎความถูกต้องและการกรองขยะ (Strict Sanitization Rules):
       ...
       - สำหรับเบอร์โทรศัพท์ (phone), เลขบัตรประชาชน (id_card_no), เลขตัวถัง (vin), และเลขเครื่องยนต์ (engine_no) ต้องประกอบด้วยตัวเลขอารบิกและตัวอักษรภาษาอังกฤษเท่านั้น ห้ามใส่เครื่องหมายลบ (-), ช่องว่าง, วงเล็บ หรือตัวอักษรไทยปะปน
       - วันที่ทั้งหมด (start_date, expiry_date, dob, registration_date, transfer_date_time, tax_expiry) ต้องส่งกลับมาเป็นรูปแบบ YYYY-MM-DD เสมอ โดยแปลงปี พ.ศ. ให้เป็น ค.ศ. (ค.ศ. = พ.ศ. - 543) ทุกครั้ง เช่น วันที่ 20 มิถุนายน 2569 ให้ส่งกลับเป็น "2026-06-20"
