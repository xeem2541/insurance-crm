...
// Delete non-motor policy
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    const [result] = await req.db.query('DELETE FROM non_motor_policies WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Policy not found' });
    }
    
    await req.db.query('INSERT INTO activity_logs (user_id, action, target_table, target_id, details) VALUES (?, ?, ?, ?, ?)',
      [req.user.id, 'DELETE', 'non_motor_policies', req.params.id, `Deleted non-motor policy ID ${req.params.id}`]);
