...
async function testGeminiKey(apiKey) {
  ...
  try {
    ...
    throw {
      status: status === 400 || status === 401 || status === 403 ? 400 : 500,
      response: {
        success: false,
        durationMs: duration,
...
async function extractDocumentData(apiKey, imageParts) {
      ...
      if (status === 401 || status === 403) {
        throw {
          ...
          response: {
            ...
            message: `Gemini API Key ไม่ถูกต้อง หมดอายุ หรือไม่มีสิทธิ์เข้าถึง (Status ${status}): ${error.response?.data?.error?.message || error.message}`,
  ...
  try {
    ...
    throw {
      status: 500,
      response: { 
        error: 'Failed to parse AI response into JSON. Please try again with a clearer image.',
        rawText: jsonText 
      }
    };
  }
  ...
  return { parsedData, usedModelName };
}
